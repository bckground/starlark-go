...
def test_error_tags_type():
    errs = error_tags("Err")
    assert.eq(type(errs), "error_tags")

test_error_tags_type()
...
def test_error_tags_merge():
    ...
    assert.eq(type(merged_plus), "error_tags")
...
test_error_tags_merge_preserves_left()

# Test merging with empty error tag set.
def test_error_tags_merge_empty():
    errs = error_tags("Err")
    empty = error_tags()
    # Test | operator
...
test_error_tags_merge_empty()

# Test chained merging.
def test_error_tags_merge_chain():
    errs1 = error_tags("A")
    errs2 = error_tags("B")
    errs3 = error_tags("C")
    # Test | operator
