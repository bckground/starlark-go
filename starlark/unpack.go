...
func UnpackArgs(fnname string, args Tuple, kwargs []Tuple, pairs ...any) error {
	...
	if len(args) > nparams {
		return &ArgumentError{msg: fmt.Sprintf("%s: got %d arguments, want at most %d",
	...
	for i, arg := range args {
		...
		if err := UnpackArg(arg, pairs[2*i+1]); err != nil {
			return &ArgumentError{msg: fmt.Sprintf("%s: for parameter %s: %s", fnname, name, err)}
		}
	}
...
kwloop:
	for _, item := range kwargs {
		...
		for i := range nparams {
			...
			if pName == string(name) {
				// found it
				if defined.set(i) {
					return &ArgumentError{msg: fmt.Sprintf("%s: got multiple values for keyword argument %s",
						fnname, name)}
				}
	...
	for i := range nparams {
		...
		if !defined.get(i) {
			return &ArgumentError{msg: fmt.Sprintf("%s: missing argument for %s", fnname, name)}
		}
	}
...
func UnpackPositionalArgs(fnname string, args Tuple, kwargs []Tuple, min int, vars ...any) error {
	if len(kwargs) > 0 {
		return &ArgumentError{msg: fmt.Sprintf("%s: unexpected keyword arguments", fnname)}
	}
	max := len(vars)
	if len(args) < min {
		var atleast string
		...
		return &ArgumentError{msg: fmt.Sprintf("%s: got %d arguments, want %s%d", fnname, len(args), atleast, min)}
	}
	if len(args) > max {
		var atmost string
		if max > min {
		...
		return &ArgumentError{msg: fmt.Sprintf("%s: got %d arguments, want %s%d", fnname, len(args), atmost, max)}
	}
	for i, arg := range args {
		if err := UnpackArg(arg, vars[i]); err != nil {
			return &ArgumentError{msg: fmt.Sprintf("%s: for parameter %d: %s", fnname, i+1, err)}
		}
	}
	return nil
}
